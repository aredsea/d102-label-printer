...
(function () {
   ...
   * ----------------------------------------------------------------------- */
  // 저장소 어댑터: window.__UB_STORAGE(read/write/reset) 가 있으면 그것을 사용
  // (프로그램의 WebView2 = .NET layout.json 브리지), 없으면 localStorage(확장).
  function readSaved() {
    if (window.__UB_STORAGE && typeof window.__UB_STORAGE.read === 'function') {
      try { return window.__UB_STORAGE.read(); } catch (e) { return null; }
    }
    try { const r = localStorage.getItem(window.UBCFG.storageKey); return r ? JSON.parse(r) : null; }
    catch (e) { return null; }
  }
  function writeSaved(slim) {
    if (window.__UB_STORAGE && typeof window.__UB_STORAGE.write === 'function') {
      try { window.__UB_STORAGE.write(slim); return true; } catch (e) { console.error('[UB] 저장 실패:', e); return false; }
    }
  ...
  function resetLayout() {
    if (window.__UB_STORAGE && typeof window.__UB_STORAGE.reset === 'function') {
      try { window.__UB_STORAGE.reset(); } catch (e) {}
      return;
    }
