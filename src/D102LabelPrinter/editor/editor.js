...
(function () {
  ...
  function open(dataArr, opts) {
    ...
    const startEdit = firstRun || !!opts.editMode;
    state = {
      root, data: dataArr, layout: window.UBLabel.getLayout(), idx: 0,
      editMode: startEdit, selKey: startEdit ? window.UBLabel.getLayout()[0].key : null,
      zoom: 1, opts: opts
    };
    ...
    if (opts.hideClose) {
      const cb = root.querySelector('[data-act=close]'); if (cb) cb.style.display = 'none';
    }
    if (typeof opts.testPrint === 'function') {
      const printBtn = root.querySelector('[data-act=print]');
      if (printBtn) {
        const tb = document.createElement('button');
        tb.className = 'ub-ed-btn primary'; tb.textContent = '테스트 인쇄';
        tb.addEventListener('click', () => { try { opts.testPrint(); } catch (e) {} });
        printBtn.parentNode.insertBefore(tb, printBtn);
      }
    }
